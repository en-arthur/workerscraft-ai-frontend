...
export default function LivePreview({ previewUrl, applicationType, onElementClick, onSandboxExpired }) {
  ...
  const handleIframeLoad = () => {
    console.log('[LivePreview] Iframe loaded successfully:', previewUrl);
    setIsLoading(false);
    setError('');
  };

  const handleIframeError = (e) => {
    console.error('[LivePreview] Iframe error:', e);
    setIsLoading(false);
    setError('Failed to load preview. The sandbox may have expired or the app may not be running.');
  };
